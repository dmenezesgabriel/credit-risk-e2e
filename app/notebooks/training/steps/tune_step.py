...
for model_name in ["catboost", "lightgbm", "xgboost"]:
    ...
    logger.info(f"Tuning {model_name} ({args.n_trials} trials) baseline={baseline_auc:.4f}")
    ...
    logger.info(f"  {model_name}: {baseline_auc:.4f} → {best_auc:.4f} ({improvement:+.4f})")
...
logger.info(f"Champion: {champion_name} val_auc={champion_meta['val_auc']:.4f}")
